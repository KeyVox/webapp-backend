...
import { createCall } from '../../lib/callData';
	...
	getAccountById(idAccount)
		.then((account) => {
			if (account) {
				...
				getRandomActivationWordByAccount(idAccount)
					.then((activationWords) => {
						if (
							activationWords != null &&
							activationWords.length > 0
						) {
							createIdentificationRequest({
								idAccount,
								idActivationWord: activationWords[0]._id,
								source: req.body.source,
								date: new Date(),
								status: 0,
							})
								.then((identificationRequest) => {
									if (identificationRequest != null) {
										createCall({
											idAccount,
											idIdentificationRequest:
												identificationRequest._id,
											status: 0,
											...
											.then((call) => {
												if (call) {
													...
													resp.status(501).end({
														...
														code: 501,
														description:
															'No se inserto la llamada',
													});
												}
											})
											.catch((errorCall) => {
												resp.status(errorCall.code).end(
													errorCall
												);
											});
									} else {
										resp.status(501).end({
											...
											code: 501,
											description:
												'No se inserto la solicitud de identificacion',
										});
									}
								})
								.catch((errorIdentificationRequest) => {
									resp.status(
										errorIdentificationRequest.code
									).end(errorIdentificationRequest);
								});
						} else {
							resp.status(404).end({
								status: 'Error',
								code: 404,
								description:
									'No se encontro ninguna huella de voz',
							});
						}
					})
				...
				resp.status(404).end({
					status: 'Error',
					code: 404,
					description: 'No se encontro ninguna cuenta',
				});
			}
		})
